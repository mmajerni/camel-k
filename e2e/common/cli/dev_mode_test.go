...
func TestRunDevMode(t *testing.T) {
	...
	WithNewTestNamespace(t, func(ns string) {
		...
		t.Run("Dev mode resource file generated configmap", func(t *testing.T) {
			...
			kamelRun := KamelWithContext(ctx, "run", "-n", ns, file, "--dev", "--resource", fmt.Sprintf("file:file:%s@/tmp/file.txt", tmpFile.Name()))
			kamelRun.SetOut(pipew)
			...
			args := os.Args
			defer func() { os.Args = args }()
			os.Args = []string{"kamel", "run", "-n", ns, file, "--dev", "--resource", fmt.Sprintf("file:file:%s@/tmp/file.txt", tmpFile.Name())}
			go kamelRun.Execute()
